...
library(synapser)
synLogin(email='nur375@psu.edu', password=, rememberMe=T)
```
...
write.csv(flight1.cog.survey.scoredcog, "~/Github/tmp/M2C2_cog_scored.csv", row.names=F)

write.csv(flight1.beep.survey.scoredcog, "~/Github/tmp/M2C2_beep_scored.csv", row.names=F)

```

```{r}
library(dataMaid)
makeDataReport(flight1.beep.survey.scoredcog, replace=T, mode=c("summarize", "check"))
```
